...
public class InputTest {
    ...
    @Test(timeout = 100)
    public void given_user_input_when_correct_then_ok() {
        ...
        String output = input.get("", validador, customCaster, false);
        assertEquals("Hello", output);
    }

    @Test(timeout = 5000)
    public void given_user_input_when_incorrect_and_correct_after_then_ok() {
        ...
        Input input = new Input(new Scanner(inputStream));
        int actual = input.get("", validador, customCaster, true);
        int expected = 1;
